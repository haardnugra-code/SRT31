...
// Replace recommendation sections 1, 2, 3 logic for MMPI
const recsOld = `  doc.setFont('helvetica', 'bold');
  doc.setFontSize(8);
  doc.setTextColor(30, 41, 59);
  doc.text('1. Untuk Wali Asuh di Asrama:', margin + 2, y);`;

const recsNew = `  doc.setFont('helvetica', 'bold');
  doc.setFontSize(8);
  doc.setTextColor(30, 41, 59);
  
  if (assessment.testType === 'mmpi_tni_polri') {
...
// The old content continues with `y += 3.5;` after `doc.text('1. Untuk Wali Asuh di Asrama:', margin + 2, y);`. We need to carefully inject the `} else {` part so we don't break the code syntax.

fs.writeFileSync('src/services/psychologicalPdfGenerator.ts.patch', content);
