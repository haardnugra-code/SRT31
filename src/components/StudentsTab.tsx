...
export const StudentsTab: React.FC<StudentsTabProps> = ({
  ...
  const [activeSubView, setActiveSubView] = useState<'table' | 'profile'>(initialSubView);
  const [selectedProfileStudentId, setSelectedProfileStudentId] = useState<string>(
    initialStudentId || students[0]?.id || ''
  );

  useEffect(() => {
    if (initialStudentId) {
      setSelectedProfileStudentId(initialStudentId);
    }
  }, [initialStudentId]);
  ...
  const handleSelectProfile = (studentId: string) => {
    ...
    if (onOpenProfileForStudent) {
      onOpenProfileForStudent(studentId);
    }
  };
  ...
  const handleOpenEditModal = (student: Student) => {
    ...
    setFormGender((student.gender as any) || 'Laki-Laki');
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto">
      {/* Mode Sub-Tab Switcher: Data Siswa & Profil Siswa */}
      <div className="no-print bg-white p-2.5 rounded-2xl border border-slate-200 shadow-sm flex items-center justify-between gap-3 flex-wrap">
        <div className="flex items-center gap-2">
          ...
          >
            <Users className="w-4 h-4 text-amber-400" /> Data & Induk Siswa ({students.length})
          </button>
          <button
            type="button"
          ...
          >
            <UserCheck className="w-4 h-4 text-white" /> Profil & Portofolio Siswa Terpadu
          </button>
        </div>
        ...
        {activeSubView === 'table' && (
          <button
            ...
            className="bg-red-600 text-white text-xs font-bold px-4 py-2 rounded-xl hover:bg-red-700 shadow transition active:scale-95 flex items-center gap-1.5 ml-auto"
          >
      ...
      {activeSubView === 'profile' ? (
        <StudentProfileTab
          ...
          prayerAttendance={prayerAttendance || []}
          config={config}
          initialStudentId={selectedProfileStudentId}
          onBackToTable={() => setActiveSubView('table')}
            ...
            return (
              ...
              >
                ...
                <div className="flex items-start gap-3.5 relative z-10 min-w-0 pt-3">
                  <div className="w-12 h-12 rounded-xl bg-slate-100 border border-slate-200 overflow-hidden flex items-center justify-center font-bold text-base flex-shrink-0">
                    {s.photo ? (
                      <img src={s.photo} alt={s.name} className="w-full h-full object-cover" />
                    ) : (
                      <div className="w-full h-full bg-red-50 text-red-600 flex items-center justify-center font-black text-lg">
                  ...
                  <div className="min-w-0 space-y-0.5 pr-10">
                    <h3
                      ...
                      className="font-bold text-slate-800 text-sm leading-snug truncate hover:text-red-600 cursor-pointer"
                      title={s.name}
                    >
                      {s.name}
                    </h3>
            ...
            <div className="overflow-y-auto flex-1 p-5 space-y-5">
              ...
              <div className="flex flex-col sm:flex-row items-center justify-between gap-3 border-b border-slate-200 pb-3">
                ...
                <div className="flex items-center gap-2 w-full sm:w-auto justify-end flex-wrap">
                  <button
                    onClick={() => {
                      generateStudentViolationHistoryPDF(selectedStudentForHistory, studentViolations, config);
                      onShowToast('Mencetak Historis', `Membuat Buku Historis Pelanggaran & Disiplin ${selectedStudentForHistory.name}...`, 'success');
                    }}
                    className="bg-rose-700 hover:bg-rose-800 text-white text-xs font-bold px-3 py-2 rounded-lg transition shadow-sm flex items-center gap-1"
                    title="Cetak Buku Historis Pelanggaran & Catatan Disiplin Siswa (PDF Resmi)"
                  >
                    <FileText className="w-3.5 h-3.5 text-rose-200" /> Cetak Buku Historis (PDF)
                  </button>
                  <button
                    onClick={() => {
